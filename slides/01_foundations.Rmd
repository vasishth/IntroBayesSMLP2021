...
The way we will conduct data analysis is as follows. 
  ...
  fake-data simulation, *prior predictive* and *posterior predictive* checks.
...
Adding a \textbf{normalizing constant} makes the above kernel density a pdf.
...
If we have $k$ successes in $n$ trials, given a success probability $p$ for each trial. If $k \sim Bin(n,p)$.
...
```###pmf:
dbinom(k, size, prob, log = FALSE)
### cdf:
pbinom(k, size, prob, lower.tail = TRUE, log.p = FALSE)
### quantiles:
...
Let $\lambda$ be the average number of events in a given time interval $[0,1]$. Let the random variable $X$ count the number of events occurring in the interval. Then: 
